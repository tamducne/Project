 ...
 */
package com.project.project1.filter;
...
public class AdminFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        //get về req, resp, session
        HttpServletRequest req = (HttpServletRequest) request;
        HttpServletResponse resp = (HttpServletResponse) response;
        HttpSession session = req.getSession();
        //lấy về account ở session
        Account account = (Account) session.getAttribute(Constant.SESSION_ACCOUNT);
        //kiểm tra xem account có != null hay không
        if (account == null) {
            resp.sendRedirect(req.getServletContext().getContextPath()+"/authen?action=login");
        }else{//làm thế này để khi tài khoản user đăng kí vào thì chỉ dùng được trong trang home, chuyển sang không được trang admin, chỉ có accoount admin mới dùng được
        //kiểm tra xem role account có phải admin hay không
            if (account.getId() != Constant.ROLE_ADMIN) {
